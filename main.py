...
SampleRate = "1h"
st.header("Turlock AOD")  # Fixed assignment of header
...
if df_1 is not None:
    ...
    if 'AOD_440nm' in df_1.columns and 'AOD_500nm' in df_1.columns and 'AOD_675nm' in df_1.columns:
        ...
        if st.button("Submit"):
            ...
            ax.set_title("Wind Vector and AOD")  # Title for combined plot
            ax.set_xlabel("Time")
            ax.set_ylim(AOD_min, AOD_max)
            ax2 = ax.twinx()

            # Wind data processing and plotting
            ...
            ax.quiver(
                WNDdf[5].resample('3h').mean().index, maxWind - 1,
                -WNDdf[5].resample('3h').mean().div(10),  # X-component of arrows
                -WNDdf[6].resample('3h').mean().div(10),  # Y-component of arrows
                color='b',
                label='Wind Vector'
            )
